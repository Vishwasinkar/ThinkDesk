...
// ✅ New: Get all classrooms (available to show on homepage)
router.get('/available', async (req, res) => {
    console.log('🔵 /class/available route hit');
    try {
        const classrooms = await Classroom.find().populate('owner', 'name email');

        if (!classrooms || classrooms.length === 0) {
            return responseFunction(res, 404, 'No classrooms available', null, false);
        }

        return responseFunction(res, 200, 'Available classrooms fetched successfully', classrooms, true);
    } catch (err) {
        console.log('Error fetching available classrooms:', err);
        return responseFunction(res, 500, 'Internal server error', err, false);
    }
});
